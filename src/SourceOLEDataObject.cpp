...
FindDataEntry:
	...
	if(!pDataEntry->pData) {
		...
		if(properties.pOwnerRTB) {
			properties.pOwnerRTB->Raise_OLESetData(static_cast<IOLEDataObject*>(this), formatID, pFormat->lindex, pFormat->dwAspect);
		}
...
FindDataEntry:
	...
	if(!pDataEntry->pData) {
		if(secondChance) {
			if(properties.pOwnerRTB && properties.pOwnerRTB->dragDropStatus.IsDragging()) {
				return S_OK;
			} else {
				// format not supported
				return DV_E_FORMATETC;
	...
	if(properties.numberOfItemsToDisplay > 1) {
		...
		if(pEntry) {
			...
			*(pDataEntry->pFormat) = *pFormat;
			...
			if(properties.pOwnerRTB) {
				properties.pOwnerRTB->Raise_OLEReceivedNewData(static_cast<IOLEDataObject*>(this), formatID, pFormat->lindex, pFormat->dwAspect);
			}
...
void SourceOLEDataObject::SetOwner(RichTextBox* pOwner)
{
	if(properties.pOwnerRTB) {
	...
	switch(format.cfFormat) {
		...
		if(pEntry) {
			...
			*(pDataEntry->pFormat) = format;
			...
			if(properties.pOwnerRTB) {
				properties.pOwnerRTB->Raise_OLEReceivedNewData(static_cast<IOLEDataObject*>(this), formatID, format.lindex, format.dwAspect);
			}
