...
{
	ATLASSERT(expectedVarType == VT_BOOL || expectedVarType == VT_DATE || expectedVarType == VT_I2 || expectedVarType == VT_I4 || expectedVarType == VT_R4 || expectedVarType == VT_R8);
	...
	if(SUCCEEDED(hr)) {
		...
		switch(pVariant->vt) {
			...
			case VT_I4:
				...
				break;
			case VT_R4:
				hr = pStream->Read(&pVariant->fltVal, sizeof(FLOAT), NULL);
				break;
			case VT_R8:
				hr = pStream->Read(&pVariant->dblVal, sizeof(DOUBLE), NULL);
				break;
		}
	}
	return hr;
...
{
	ATLASSERT(pVariant->vt == VT_BOOL || pVariant->vt == VT_DATE || pVariant->vt == VT_I2 || pVariant->vt == VT_I4 || pVariant->vt == VT_R4 || pVariant->vt == VT_R8);
	...
	if(SUCCEEDED(hr)) {
		switch(pVariant->vt) {
			...
			case VT_I4:
				...
				break;
			case VT_R4:
				hr = pStream->Write(&pVariant->fltVal, sizeof(FLOAT), NULL);
				break;
			case VT_R8:
				hr = pStream->Write(&pVariant->dblVal, sizeof(DOUBLE), NULL);
				break;
		}
	}
	return hr;
...
BOOL IsValidTextParagraphTabIndex(int tabIndex, ITextPara* pTextParagraph)
{
	long tabCount = 0;
	if(pTextParagraph && SUCCEEDED(pTextParagraph->GetTabCount(&tabCount))) {
		return (0 <= tabIndex && tabIndex < tabCount);
	}
	return FALSE;
}

BOOL IsValidTextParagraphTabPosition(float tabPosition, ITextPara* pTextParagraph)
{
	if(pTextParagraph) {
		long tabAlign = tomAlignLeft;
		long leaderChar = tomSpaces;
		HRESULT hr = pTextParagraph->GetTab(tomTabHere, &tabPosition, &tabAlign, &leaderChar);
